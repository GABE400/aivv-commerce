...
export function ProductForm({ categories, suppliers, initialData, productId }: ProductFormProps) {
    ...
    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {uploadedImages.map((url, i) => (
                    <div key={i} className="relative aspect-square rounded-xl glass border border-glass-border overflow-hidden group">
          ...
          <Card className="glass border-glass-border">
            ...
            <CardContent className="p-6 space-y-6">
              {variants.map((_: any, index: number) => (
                <div key={index} className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 rounded-xl bg-muted/30 border border-glass-border relative">
                   <div className="space-y-2">
                      <Label className="text-[10px] uppercase font-bold text-muted-foreground">Name</Label>
                      <Input {...form.register(`variants.${index}.name` as const)} placeholder="S, M, Red, etc" />
                   </div>
                   <div className="space-y-2">
                      <Label className="text-[10px] uppercase font-bold text-muted-foreground">SKU</Label>
                      <Input {...form.register(`variants.${index}.sku` as const)} placeholder="UNQ-ID-001" />
                   </div>
                   <div className="space-y-2">
                      <Label className="text-[10px] uppercase font-bold text-muted-foreground">Price</Label>
                      <Input {...form.register(`variants.${index}.price` as const)} placeholder="29.99" />
                   </div>
                   <div className="flex items-end justify-between gap-4">
                      <div className="space-y-2 flex-1">
                        <Label className="text-[10px] uppercase font-bold text-muted-foreground">Inventory</Label>
                        <Input type="number" {...form.register(`variants.${index}.inventory` as const, { valueAsNumber: true })} />
                      </div>
                      <Button 
                        type="button" 
                        variant="ghost" 
                   ...
                   {productType === "digital" && (
                     <div className="md:col-span-4 pt-2">
                        <Label className="text-[10px] uppercase font-bold text-muted-foreground mb-2 block">Source Digital Asset</Label>
                        <FileUploader 
                          isImage={false}
                          accept=".pdf,.zip,.rar,.mp4,.mp3,.png,.jpg"
                          label="Upload Digital Master"
                          preview={form.watch(`variants.${index}.assetUrl`)}
                          onUploadSuccess={(url) => form.setValue(`variants.${index}.assetUrl`, url)}
                          onUploadError={() => {}}
                        />
                     </div>
                   )}
        ...
        <div className="space-y-8">
          ...
          <Card className="glass border-glass-border">
            <CardContent className="p-6 space-y-4">
              <h3 className="text-[10px] uppercase font-bold text-muted-foreground tracking-widest">Live Preview</h3>
              <div className="space-y-2">
                <p className="text-sm font-bold truncate">{form.watch("name") || "Product Name"}</p>
                <p className="text-xs text-muted-foreground font-mono truncate">/products/{form.watch("slug") || "your-slug"}</p>
