...
class CJDropshippingClient {
  ...
  private async getAccessToken(): Promise<string> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`CJ Auth request failed with status: ${response.status}`);
      }

      const res = (await response.json()) as any;
      if (res.code !== 200 || !res.data?.accessToken) {
        throw new Error(`CJ Auth returned failed body: ${res.message || JSON.stringify(res)}`);
      }
  ...
  private async fetchCJ(path: string, options: RequestInit = {}) {
    ...
    const res = (await response.json()) as any;
    if (res.code !== 200) {
      throw new Error(`CJ API returned error code ${res.code}: ${res.message || JSON.stringify(res)}`);
    }

    return res;
  }

  async getProducts(params: { page?: number; size?: number; keyWord?: string } = {}) {
  ...
  async calculateShippingRates(params: CJCalculateShippingParams): Promise<number> {
    try {
      const payload = {
      ...
      if (Array.isArray(data) && data.length > 0) {
        const validOptions = data.filter((opt: any) => typeof opt.logisticPrice === "number" || typeof opt.logisticPrice === "string");
        if (validOptions.length > 0) {
          const prices = validOptions.map((opt: any) => parseFloat(opt.logisticPrice));
